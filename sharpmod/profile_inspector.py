...
def inspect_profile(collection) -> list[QualityFinding]:
    ...
    supersaturated = int(
        np.sum(dwpc[:paired][thermo_mask] > tmpc[:paired][thermo_mask] + 0.5)
    )
    if supersaturated:
        findings.append(
            QualityFinding(
    ...
    for field, label in (
        ...
        if missing / expected > 0.25:
            findings.append(
                QualityFinding(
                    "warning",
                    f"missing-{label.replace(' ', '-')}",
                    f"More than 25% of {label} values are missing.",
                )
            )

    requested = (_meta(collection, "requested_lat"), _meta(collection, "requested_lon"))
    selected = (_meta(collection, "selected_lat"), _meta(collection, "selected_lon"))
    try:
...
def profile_summary(collection) -> list[str]:
    ...
    if vorticity is None:
        ...
        try:
            ...
            value = str(vorticity)
        lines.append(
            f"Surface relative vorticity: {value}" + (f" ({source})" if source else "")
        )
    return lines
