...
A reference manual for every thermodynamic, kinematic, and composite parameter
calculated and displayed in the SHARPpy Reimagined Skew-T sounding analysis
system. Each entry gives the physical meaning, the formula as implemented, any
edge cases or clamps applied in code, the colour thresholds used to draw it, and
the literature reference.

**Where these appear.** Most values are drawn in the IndexBoard and the inset
product panels to the right of and below the Skew-T. The same numbers appear in
`sharpmod-render` PNG output, because the GUI and the renderer share one canvas.
Double-click the lower-left parcel inset to change which parcel the Skew-T
traces; the parcel-dependent parameters in section 1 follow that selection.

**Related documents.** For interaction — zoom, editing, keyboard shortcuts — see
**Help → Controls and Shortcuts** (`F1`) in the sounding window, or the
[README's GUI section](README.md#desktop-gui). For how a sounding is fetched and
what provenance is recorded, use **Data → Source & Quality Inspector…**.

**A note on units.** Formulae below are written in the units the calculation
uses internally, which are not always the units displayed. Temperature, wind,
and precipitable-water display units are configurable in
**File → Preferences**; the underlying computation is unaffected.

---

## Contents

1. [Convective / thermodynamic parcel parameters](#1-convective--thermodynamic-parcel-parameters)
   — CAPE, CIN, LCL, LFC, EL, LI
2. [Convective / thermodynamic column parameters](#2-convective--thermodynamic-column-parameters)
   — PWAT, MeanW, LowRH, MidRH, DCAPE, DownT, K, TT, ConvT, MaxT, ESP, WNDG,
   TEI, 3CAPE/6CAPE, MBURST, SigSvr, lapse rates
3. [Severe weather composite parameters](#3-severe-weather-composite-parameters)
   — SCP, STP(cin), STP(fix), SHIP, DCP
4. [Kinematic parameters](#4-kinematic-parameters)
   — SRH, bulk shear, mean wind, storm-relative wind, BRN shear, Bunkers,
   Corfidi, streamwiseness
5. [Composite board / specialty parameters](#5-composite-board--specialty-parameters)
   — EHI, VGP, Peskov, MCS index, SWEAT, MOSHE, LRGHAIL, HGZ CAPE, NSTP, NCAPE,
   ECAPE, LSCP, WBZ height
6. [Parameter colour-coding and interpretation tiers](#6-parameter-color-coding--interpretation-tiers)
7. [Parcel lifting and moist/dry adiabatic theory](#7-parcel-lifting-and-moistdry-adiabatic-theory)
8. [The effective inflow layer concept](#8-the-effective-inflow-layer-concept)
...
All of the values below come from `sharpmod/colors.py`, which is deliberately
Qt-free so the thresholds stay testable. Three implementation details are worth
knowing before reading the tables:

*   **Colors are recomputed at draw time** from the current value, never reused
    from a stale default. Editing the profile re-colours every affected index
    immediately.
*   **Missing values never raise.** A NumPy masked constant, `None`, `"--"`, or a
    non-finite float maps to the neutral foreground color — or, for the
    alert-scale parameters, to the documented lowest tier — matching legacy
    behavior.
*   **Two palettes exist.** The tables below give the *modernized* scheme. The
    legacy scheme is retained as `ALERT_TIERS_LEGACY` for comparison. Only three
    colors changed, all because the legacy value was close to unreadable against
    the black chart background:

| Element | Legacy | Modernized |
| --- | --- | --- |
| SARS non-tornadic match | `#996600` | `#c9a24b` |
| Alert tier 1 | `#775000` | `#c8911f` |
| Alert tier 2 | `#996600` | `#e0a800` |
| Alert tier 6 | `#ad00e7` (dark purple) | `#ff00ff` (bright pink) |

The system uses a modernized **Alert Palette** of 7 steps, indexed `0..6`:

*   **Tier 0:** `#775000` (Muted Dark Gold / lowest, SCP-left, or missing)
...
On the dark palettes (Standard and Protanopia) these values are used exactly as
listed. On the **Inverted** (light) palette each one is checked against the light
background first: colors that already clear a 4.5:1 contrast ratio are left
alone, and only those that fall short are minimally darkened toward black. So a
tier keeps its hue and its ordering on a light canvas, and only changes at all
when it would otherwise be hard to read. Plotted scientific values, units, and
established dark-theme trace colors are unchanged by the palette switch.
...
## 9. Where Each Parameter Is Implemented

The calculations in this document are not spread across the codebase. If you
need to read or verify one, these are the files that own it:

This is a fork, so ownership is split. The classic SPC composites are inherited
from vendored upstream SHARPpy and are *not* reimplemented here; the fork's own
modules add parameters upstream lacks, override specific ones, and supply the
optimized kernels. Knowing which is which matters when tracing a value:

| Concern | Owner |
| --- | --- |
| Effective inflow layer, SCP, STP(cin), STP(fix), SHIP, DCAPE, WNDG, ESP, TEI, MBURST, SHERB, LHP, EHI, and the parcel machinery | vendored `sharppy/sharptab/params.py` |
| Thermodynamic primitives: virtual temperature, mixing ratio, wet bulb, theta-e | vendored `sharppy/sharptab/thermo.py` |
| Layer-derived thermodynamic parameters | `sharpmod/sharptab/params.py` |
| Composite derived parameters, including this fork's DCP and normalized CAPE | `sharpmod/sharptab/derived.py` |
| Entraining CAPE (ECAPE), after Peters et al. (2023) | `sharpmod/sharptab/ecape.py` |
| Kinematics: bulk shear, mean wind, SRH, Bunkers storm motion | `sharpmod/sharptab/winds.py` |
| Cached coarse-grained parcel diagnostics | `sharpmod/sharptab/parcels.py` |
| The reformulated Possible Hazard Type classifier | `sharpmod/sharptab/hazard.py` |
| Profile interpolation helpers | `sharpmod/sharptab/interp.py` |
| Profile construction and the derived-attribute contract | `sharpmod/sharptab/profile.py` |
| Missing-value constants and masking rules | `sharpmod/sharptab/constants.py` |
| Tier thresholds and draw-time colour resolution | `sharpmod/colors.py` |
| Optimized Python and Rust kernels for parcels, kinematics, and DCAPE | `sharpmod/backends/` |

The vendored package is never edited in place. Where upstream behaviour had to
change for Qt6 or for a corrected calculation, it is adapted through the ordered
patch registry in `sharpmod/render_patch_registry.py`, applied by
`sharpmod/render.py`, together with the shims in `sharpmod/viz/_qt6_compat.py`.
The upstream source therefore stays a clean, readable reference.

Two properties of the implementation are enforced by tests rather than
convention, and are worth relying on:

*   **Decoder-agnostic results.** The same profile decoded through different
    input paths must produce the same derived parameters, within documented
    floating-point tolerances.
*   **Backend equivalence.** The Rust and Python kernels are held to the same
    numerical results, with Python treated as the oracle. If a native operation
    is unavailable, the calculation silently falls back to Python rather than
    returning a different answer.

Both are covered by the property lane (`python scripts/run_test_lane.py
property`), alongside checks that derived values stay inside physical ranges,
that reads are idempotent, and that masking one input degrades cleanly without
corrupting other profiles.

---

## 10. Reading a Sounding: Suggested Order

The parameter list above is organised by category, which is not the order a
forecaster uses it in. A practical sequence:

1. **Is there instability, and where does it come from?** Check MUCAPE and which
   parcel is most unstable. If the MU parcel is elevated, the surface-based
   numbers describe air the storm may never ingest — switch to the effective
   layer parameters (section 8) rather than the fixed-layer ones.
2. **Can it be released?** CIN, the cap strength, and ConvT against the forecast
   MaxT. Strong CAPE under a strong cap is a different forecast from the same
   CAPE uncapped.
3. **How will it be organised?** Effective bulk shear and SRH, then SCP. Bulk
   shear below roughly 10 m/s does not support supercells regardless of CAPE,
   which is why the SCP shear term zeroes out there.
4. **What is the dominant hazard?**
   *   Tornado: STP(cin) with the effective layer, low-level SRH, LCL height,
       3CAPE, and streamwiseness in the lowest 1–3 km.
   *   Hail: SHIP, HGZ CAPE, WBZ height, and the 700–500 hPa lapse rate.
   *   Wind: DCAPE, DownT, WNDG, DCP, and mid-level dryness (MidRH).
5. **Will it persist?** MCS maintenance probability, deep-layer mean wind, and
   Corfidi vectors for upscale growth and propagation.
6. **Sanity-check the input.** Before trusting any of it, open **Data → Source &
   Quality Inspector…** — a sounding with few levels, a missing surface record,
   or interpolated moisture will produce confident-looking numbers from thin
   data.

None of these indices is a forecast on its own. Each was derived from a specific
climatology and verification dataset, cited in its entry above; values far
outside the range a parameter was developed against should be read as "this
environment is unusual", not as a proportionally larger threat.
